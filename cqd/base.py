...
class CQD(nn.Module):
    ...
    def score_candidates(self,
        ...
        lhs_emb = self.embeddings[0](triples[:, 0]) #(B, d)
        rel_emb = self.embeddings[1](triples[:, 1]) #(B, d)
        rhs_emb = self.embeddings[0](triples[:, 2]) #(B, d)
        to_score = self.embeddings[0].weight  #(E, d)
        scores_o, _ = self.score_o(lhs_emb, rel_emb, to_score) #得到正向预测出的所有实体概率
        scores_s, _ = self.score_s(to_score, rel_emb, rhs_emb) #得到反向预测出的所有实体概率
